...
#        model = TCN(input_size=1, output_size=33, num_channels=[32] *12, kernel_size=2)
    ...
    train_labeled_set, train_unlabeled_set, train_unlabeled_set2,val_set, test_set = get_cifar10('./data', args.n_labeled,
    ...
    if args.resume:
        ...
        logger = Logger(os.path.join(args.out_path, 'cifar10_log_%d.txt'%args.n_labeled), title=title, resume=True)
        logger.append([args.start_epoch, 0, 0, val_loss, val_acc,test_loss, test_acc])
    else:
        logger = Logger(os.path.join(args.out_path, 'cifar10_log_%d.txt'%args.n_labeled), title=title)
