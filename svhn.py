...
#
    ...
    train_labeled_set, train_unlabeled_set, train_unlabeled_set2,val_set, test_set = get_svhn('./data', args.n_labeled,
    ...
    if args.resume:
        ...
        logger = Logger(os.path.join(args.out_path, 'svhn_log_%d.txt'%args.n_labeled), title=title, resume=True)
        logger.append([args.start_epoch, 0, 0, val_loss, val_acc,test_loss, test_acc])
    else:
        logger = Logger(os.path.join(args.out_path, 'svhn_log_%d.txt'%args.n_labeled), title=title)
